...
var s_account="cnetnews"
/* Link Tracking Config */
var s_trackDownloadLinks=true
var s_trackExternalLinks=true
var s_trackInlineStats=true
var s_linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,doc,pdf,xls"
var s_linkInternalFilters="javascript:,news.com.com,news.search.com,investor.news.com"
...
function s_doPlugins() {
	...
	s_vp_getCGI('s_prop4','q');
	s_setDetailPaging('s_pageName','s_prop6','Refer',s_getReferringDomain());
}
...
function s_setDetailPaging(orig,detail,desc,val)
	{var st_prop=s_vp_getValue(orig);if(val) st_prop= st_prop+"|"+
	desc+"-"+val;{s_vpr(detail,st_prop)}}
function s_getReferringDomain()
	{rt = s_gtfs().document.referrer;s_ssl=(s_gtfs().location.protocol
	&&s_gtfs().location.protocol.toLowerCase()=="https");if(
	rt.indexOf("http"+(s_ssl?"s":"")+"://")>-1){rt=rt.
	substring(("http"+(s_ssl?"s":"")+"://").length);}while(
	rt.indexOf("\\")>-1){rt=rt.replace("\\","/")};if(
	rt.indexOf("/")>-1){rt=rt.substring(0,rt.indexOf("/"));}
	irf=s_pt(s_gg('linkInternalFilters'),',',s_chRef,rt);
	if(!irf)return rt;return '';}function s_chRef(t,k){if(t){if
